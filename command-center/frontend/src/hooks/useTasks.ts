...
export interface UseTasks {
  ...
  loaded: boolean;
  refresh: () => void;
  add: (title: string, dueDate?: string | null, dueTime?: string | null, category?: TaskCategory | null, projectId?: number | null) => void;
...
export function useTasks(): UseTasks {
  ...
  const add = useCallback((title: string, dueDate?: string | null, dueTime?: string | null, category?: TaskCategory | null, projectId?: number | null) => {
    const trimmed = title.trim();
    if (!trimmed) return;
    api.tasks.add(trimmed, dueDate ?? null, dueTime ?? null, category ?? null, projectId ?? null).then(notifyTasksChanged).catch(() => {});
  }, []);
