...
import type { Task, TaskCategory, TaskKind, TaskPatch } from "../types";
...
export interface UseTasks {
  ...
  loaded: boolean;
  refresh: () => void;
  add: (title: string, dueDate?: string | null, dueTime?: string | null, category?: TaskCategory | null, projectId?: number | null, kind?: TaskKind | null) => void;
...
export function useTasks(): UseTasks {
  ...
  const add = useCallback((title: string, dueDate?: string | null, dueTime?: string | null, category?: TaskCategory | null, projectId?: number | null, kind?: TaskKind | null) => {
    const trimmed = title.trim();
    if (!trimmed) return;
    api.tasks.add(trimmed, dueDate ?? null, dueTime ?? null, category ?? null, projectId ?? null, kind ?? "task").then(notifyTasksChanged).catch(() => {});
  }, []);
